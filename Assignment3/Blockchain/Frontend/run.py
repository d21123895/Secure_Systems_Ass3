...
from Blockchain.Backend.util.util import endcode_base58, decode_base58
...
def transactions(txid = None):
    if txid:
        return redirect(url_for('transactiondetail', txid = txid))
    else:
        ErrorFlag = True
        while ErrorFlag:
            try:
...
def txDetail(txid):
    ...
    for block in blocks:
        for Tx in block['Txs']:
            if Tx['TxId'] == txid:
                return render_template('transactiondetail.html', Tx=Tx, block = block , encode_base58 = endcode_base58,
...
def mempool():
    try:
        ...
        #If TxId is not in the original list then remove it from the memory pool
        for txid in memoryPool:
            if txid not in mempooltxs:
                del memoryPool[txid]

        #Add the new Tx to the memory pool if it is not already there
...
def memTxDetails(txid):
    if txid in memoryPool:
        ...
        encode_base58 = endcode_base58, bytes = bytes, sha256 = sha256, main_prefix = main_prefix,
        Unconfirmed = True)
    else:
...
def search():
    ...
    if len(identifier) == 64:
        if identifier[:4] == "0000":
            ...
            return redirect(url_for('transactiondetail', txid = identifier))
    else:
...
def showBlock(blockHeader):
    ...
    for block in blocks:
        if block['BlockHeader']['blockHash'] == blockHeader:
            ...
            encode_base58 = endcode_base58, bytes = bytes, sha256 = sha256)
    ...
    if len(publicAddress) < 35 and publicAddress[:1] == "1":
        ...
        for TxId in AllUtxos:
           for tx_out in AllUtxos[TxId].tx_outs:
            if tx_out.script_public_key.cmds[2] == h160.hex():
...
def wallet():
    ...
    if request.method == "POST":
        ...
        scriptPubKey = sendCoin.script_public_key(FromAddress)
        verified = True
